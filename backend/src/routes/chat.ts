...
import { otherLanguage, translateText } from '../lib/translate';
...
chat.get('/', requireCustomer, async (c) => {
  ...
  try {
    const conversation = await getOrCreateConversation(
      ...
      String(customer.id),
      orderId,
      String(customer.preferred_language || 'ar'),
    );
...
chat.post('/messages', requireCustomer, async (c) => {
  const customer = c.get('customer')!;
  const body = await c.req.json<{ message?: string; order_id?: string }>();
  const text = (body.message || '').trim();
  if (!text) return errorResponse('Message required', 400);

  const supabase = createSupabase(c.env);
  const lang = String(customer.preferred_language || 'ar');
  try {
    const conversation = await getOrCreateConversation(supabase, String(customer.id), body.order_id || null, lang);
    const translated = await translateText(text, lang, otherLanguage(lang, lang, 'customer'));
    const { data: message, error } = await supabase
      ...
      .insert({
        ...
        is_translated: Boolean(translated && translated !== text),
      })
      .select()
      .single();
    ...
    await supabase
      .from('conversations')
      .update({ last_message_at: new Date().toISOString(), customer_language: lang })
...
chat.post('/inbox/:id/messages', requireStaff, async (c) => {
  ...
  const targetLang = conversation.customer_language || 'ar';
  const translated = await translateText(text, 'ar', targetLang);
  const { data: message, error } = await supabase
    .from('chat_messages')
    .insert({
      conversation_id: conversation.id,
