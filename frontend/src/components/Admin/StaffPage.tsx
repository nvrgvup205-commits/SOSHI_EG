    ...
    <div>
      ...
      {showForm && (
        <div className="card p-4 sm:p-6 mb-6">
          <h2 className="font-display text-xl text-fg mb-4">{t('staff.add', lang)}</h2>
          <form onSubmit={handleSave} className="space-y-4">
            <div>
              <label className="text-xs text-muted uppercase tracking-wider mb-1 block">{t('customers.name', lang)}</label>
              <input className="input-field" required value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} />
            </div>
            <div>
              <label className="text-xs text-muted uppercase tracking-wider mb-1 block">{t('customers.phone', lang)}</label>
            ...
            <div>
              <label className="text-xs text-muted uppercase tracking-wider mb-1 block">{t('staff.role', lang)}</label>
              <select className="input-field" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>
                {STAFF_ROLES.map((r) => <option key={r} value={r}>{roleLabel(r)}</option>)}
              </select>
            </div>
            <p className="text-muted text-xs">{t('staff.default_password', lang)}: <span className="text-accent font-mono">1234</span></p>
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center text-muted">{t('staff.empty', lang)}</div>
      ) : (
        <>
          {/* Mobile cards */}
              ...
              <div key={s.id} className="card p-4">
                <div className="flex justify-between items-start gap-3">
                  <div className="min-w-0">
                    <div className="text-fg font-medium">{s.full_name}</div>
                    <div className="flex items-center gap-1 text-muted text-sm mt-1" dir="ltr">
              ...
              <tbody>
                {staff.map((s) => (
                  <tr key={s.id} className="border-b border-white/5 hover:bg-white/5">
                    <td className="p-4 text-fg">{s.full_name}</td>
                    <td className="p-4 text-muted" dir="ltr">{s.phone || '—'}</td>
                    <td className="p-4"><span className="text-xs px-2 py-1 border border-accent/30 text-accent">{roleLabel(s.role)}</span></td>
                    <td className="p-4">
                      <span className={`text-xs ${s.is_active ? 'text-success' : 'text-muted'}`}>
