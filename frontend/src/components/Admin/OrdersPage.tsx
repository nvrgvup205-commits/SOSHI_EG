            ...
            <div key={order.id} className="card p-5">
              <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
                <div>
                  ...
                  <div className="text-muted text-sm mt-1">{order.customer_name} · {order.customer_phone}</div>
                  <div className="text-muted text-xs mt-1">{formatDate(order.created_at, lang)}</div>
                </div>
                <div className="text-2xl font-display text-fg">{order.total_price} {t('currency', lang)}</div>
              </div>
              <div className="flex gap-2 flex-wrap">
                {statuses.map((s) => (
                  <button key={s} onClick={() => updateStatus(order.id, s)}
                    className={`px-3 py-1 text-xs uppercase tracking-wider border transition-all ${
                      order.status === s ? 'border-accent text-accent bg-accent/10' : 'border-white/10 text-muted hover:border-white/30'
                    }`}>
                    {t(`status.${s}`, lang)}
                  </button>
                ))}
