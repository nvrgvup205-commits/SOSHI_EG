  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-muted mt-2 text-sm">{t('admin.welcome', lang)}, {(user as { full_name?: string })?.full_name}</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {cards.map((card) => (
          <div key={card.label} className="card p-4 sm:p-6">
            ...
            <div className="font-display text-2xl sm:text-4xl text-fg">{card.value}</div>
            <div className="text-muted text-[10px] sm:text-xs uppercase tracking-wider mt-1 sm:mt-2">{card.label}</div>
          </div>
        ))}
      </div>
    </div>
